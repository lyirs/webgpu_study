...
export class GLTFNode {
  ...
  buildRenderPipeline(
    ...
    this.gpuUniforms = device.createBuffer({
      size: 4 * 4 * 4,
      usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
      mappedAtCreation: true,
    });
    new Float32Array(this.gpuUniforms.getMappedRange()).set(this.transform);
    this.gpuUniforms.unmap();
    ...
    for (let i = 0; i < this.mesh.primitives.length; ++i) {
      this.mesh.primitives[i].buildRenderPipeline(
        device,
        shaderModule,
        bindGroupLayouts,
        swapChainFormat,
