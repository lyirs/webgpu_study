...
import shaderCode from "./shader/glb.wgsl?raw";
// 创建canvas
...
const camera = new Camera(vec3.create(0, 0, 300));
...
const render = () => {
  ...
  commandEncoder.copyBufferToBuffer(upload, 0, viewParamsBuffer, 0, 4 * 4 * 4);
  ...
  glbFile?.render(renderPass, viewParamsBindGroup);
  renderPass.end();
