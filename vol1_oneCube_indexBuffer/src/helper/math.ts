...
function getViewProjectionMatrix(
  ...
  const projectionMatrix = getProjectionMatrix(fov, aspect, near, far); // 创建一个透视投影矩阵
  const viewMatrix = getViewMatrix(position);
  mat4.multiply(projectionMatrix, viewMatrix, viewProjectionMatrix); //将投影矩阵和视图矩阵相乘，得到一个可以同时应用摄像机位置和投影的矩阵。
...
function getProjectionMatrix(
  ...
  mat4.perspective(fov, aspect, near, far, projectionMatrix); // 创建一个透视投影矩阵
