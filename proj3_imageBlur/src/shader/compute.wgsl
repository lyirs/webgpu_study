...
fn main(
    ...
    for (var r = 0; r < 4; r++) {
        ...
        // 4 * LocalInvocationID.x + u32(c) 是要存储的位置。
            //
            // textureSampleLevel(t,: texture_2d<f32>, s: sampler, coords: vec2<f32>, level: f32) -> vec4<f32>* 使用显式 mip 级别或 mip 级别 0 对纹理进行采样。
            // @param t:	要采样的 sampled 或 depth 纹理。
            // @param s:	samplertype.
            // @param coords:	用于采样的纹理坐标。
            // @param level:	mip 级别，级别 0 包含纹理的全尺寸版本。 对于 level 是 f32 的函数，如果格式是可过滤的，则可以在两个级别之间插入小数值根据 Textue Format Capabilities。如果未指定，则对 mip 级别 0 进行采样。
            //
    ...
    for (var r = 0; r < 4; r++) {
        for (var c = 0; c < 4; c++) {
            ...
            if center >= filterOffset && center < 128 - filterOffset && all(writeIndex < dims) {          // 确保不越界并且所有索引都在有效范围内
